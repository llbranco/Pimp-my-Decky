...
check_root_or_prompt() {
    if [[ "$EUID" -ne 0 ]]; then
        zenity --password --title="Autenticação Necessária" --text="Este script requer permissões de root para ser executado." | sudo -S true
        if [[ $? -ne 0 ]]; then
            zenity --error --title="Erro de Autenticação" --text="Falha ao autenticar como root. Saindo do script."
            exit 1
        fi
    fi
}
...
install_flatpak_apps() {
    ...
    if [[ -z "$selected_apps" ]]; then
        zenity --info --text="Nenhum aplicativo selecionado para instalação."
        return
    fi

    IFS="|" read -ra apps <<< "$selected_apps"
    for app in "${apps[@]}"; do
        zenity --info --text="Instalando $app..."
...
install_steamos_btrfs() {
    ...
    case "$compression_choice" in
        1) zenity --notification --text="Sem compressão selecionada."; return ;;
        2) compressratio=""; zenity --notification --text="Compressão padrão selecionada." ;;
        3) compressratio="5"; zenity --notification --text="Compressão média selecionada." ;;
        4) compressratio="7"; zenity --notification --text="Compressão recomendada selecionada." ;;
        5) compressratio="9"; zenity --notification --text="Compressão alta selecionada." ;;
        *) zenity --error --text="Opção inválida ou operação cancelada."; return ;;
    esac

    zenity --info --text="Instalando SteamOS-BTRFS..."
    ...
    if [[ -n "$compressratio" ]]; then
        zenity --info --text="Configurando compressão com zstd:$compressratio..."
...
install_emudeck() {
    zenity --info --text="Instalando EmuDeck..."
...
install_cryoutilities() {
    zenity --info --text="Instalando CryoUtilities..."
...
setup_spotify_spicetify() {
    zenity --info --text="Verificando instalação do Spotify Flatpak..."
    if ! flatpak list | grep -q "com.spotify.Client"; then
        zenity --info --text="Spotify não instalado. Instalando..."
        flatpak install -y flathub com.spotify.Client
    fi

    zenity --info --text="Instalando Spicetify CLI..."
    curl -fsSL https://raw.githubusercontent.com/spicetify/cli/main/install.sh | sh

    zenity --info --text="Ajustando permissões para Spicetify..."
    ...
    zenity --info --text="Atualizando configuração do Spicetify..."
    mkdir -p "$CONFIG_DIR"
    ...
    zenity --info --text="Aplicando configuração do Spicetify..."
    spicetify apply

    zenity --info --text="Instalando Spicetify Marketplace..."
    curl -fsSL https://raw.githubusercontent.com/spicetify/marketplace/main/resources/install.sh | sh

    zenity --info --text="Spotify com Spicetify configurado!"
}
...
main() {
    ...
    zenity --info --text="Instalações concluídas. Aproveite!"
}
